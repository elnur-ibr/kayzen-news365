<!-- Top Section -->
<section
    class="@if (!$themeSettings->background_color || mode()) bg-theme-four border-b border-cyan-900 @endif top_header  transition-all duration-500 z-20 ease-in dark:shadow-white/10 fixed left-0 top-0 w-full xl:relative"
    ...
    <div class="container mx-auto py-2 flex justify-between items-center">
        ...
        <button onclick="openSidebar()" type="button"
            class="xl:hidden bg-neutral-100 dark:bg-theme-three text-neutral-600 dark:text-neutral-50 rounded-full w-9 h-9 flex justify-center items-center">
        ...
        <div class="">
            <a class="w-32 h-6 flex_center overflow-hidden" href="{{ __url('/') }}">
                <img src="{{ app_setting()->footer_logo }}" alt="Brand logo light" />
            </a>
        </div>
        ...
        <section class="flex items-center gap-2 md:gap-4">
            <!-- Login / Registration -->
            @include('themes.times.components.common.login-button')

            <!-- Dark/Light Switch-->
            <button id="theme-toggle" type="button" aria-label="Use Dark Mode"
                class="btn_theme bg-neutral-100 dark:bg-theme-three text-orange-600 dark:text-white w-9 h-9 flex justify-center items-center rounded-full border dark:border-neutral-800">
            ...
            @if ($languages->count() > 1)
                <div class="hidden xl:block relative text-left">
                    <button type="button" id="lang-toggle"
                        class="text-sm bg-white border border-gray-300 rounded-lg px-2 py-1 shadow-sm hover:bg-gray-50 focus:outline-none flex gap-0.5 dark:bg-theme-three dark:border-theme-three dark:text-neutral-50">
                        🌐 <span id="lang-label">{{ $activeLanguage->langname }}</span>
                    </button>
                    <div id="lang-menu"
                        class="absolute right-0 mt-2 w-max bg-white border border-gray-200 rounded-lg shadow-lg transition-all duration-200 ease-out transform opacity-0 scale-95 translate-y-2 pointer-events-none origin-top z-10 dark:bg-theme-three dark:border-theme-three">
            ...
            <!-- Search button -->
            <div class="xl:hidden">
                @include('themes.times.components.common.search-button')
            </div>
